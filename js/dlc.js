...
// 微信跳转
// function redirect() {
//     location.href ='https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx325c92d9ef132efc&redirect_uri=' + location.href + '&response_type=code&scope=snsapi_userinfo&state=1#wechat_redirect';
// }
...
function showMask() {
    $('.mask').remove();
    $('.bigBox').append('<div class="mask1" style="position: fixed;height: 100%;width: 100%;top:0;background:rgba(0,0,0,0.6);z-index: 1;"></div>');
}
...
function dlcUrl() {
    return 'http://xdsxgzh.app.xiaozhuschool.com/';
}
    ...
    if (!token) { //获取不到token
        if (isWX_Allipay() == 'WX') { //如果是微信客户端
            if (getUrlParam('code') == '') {
                ...
                dlc_request('App/public/api/', {
                    api_name: 'wx_auth',
                    code: getUrlParam('code')
                }, function (res) {
                    if (res.code == 1) {
                        user_save(res.data.data.token);
